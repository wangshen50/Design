...
{
    public const float BUBBLE_HEIGHT = 90f;	    // 气泡高度
    public const float GAP_BETEEMN_BUBBLE = 10f;	// 气泡间的间隙

    /// <summary> 弹出对话 </summary>
    public static void PopBubble(View view, string message, AudioClip audioType)
    {
        var curSelectedGo = UnityEngine.EventSystems.EventSystem.current.currentSelectedGameObject;
        string currentButtonName = "";
        if(curSelectedGo != null)
            currentButtonName = UnityEngine.EventSystems.EventSystem.current.currentSelectedGameObject.name; // 获得点击的按钮的名字
    ...
    public static void MovePanel(View view, float heightOffset )
    {
        ...
        }else
        {
            for (int i = 0; i < view.m_popedChatBubbles.Count; i++)
            {
                ...
                view.m_popedChatBubbles[i].GetComponent<RectTransform>().anchoredPosition = new Vector2(posX, posY + heightOffset);
            }
        }
    }

    /// <summary> 计算新插入的Bubble的posY </summary>
    public static float GetNewBubblePosY(View view, float newBubbleHeight)
    {
        float FIRST_BUBBLE_POSY = 209f;
        if (view.m_popedChatBubbles.Count < view.m_templateButton.Count)
        {
    ...
    public static void ScrollUpBubbles(View view, float height)
    {
